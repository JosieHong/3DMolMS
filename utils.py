...
LastEditors: yuhhong
LastEditTime: 2022-12-10 00:02:01
'''
import gzip
import pandas as pd
...
from dataset import MERGEDataset, CSVDataset
...
def load_data(data_path, mol_path, num_atoms, out_dim, resolution, dataset, num_workers, batch_size, data_augmentation, shuffle): 
    if dataset == 'merge': 
        supp = mgf.read(data_path) # ms data
        gzsupp = Chem.ForwardSDMolSupplier(gzip.open(mol_path)) # mol data 
        dataset = MERGEDataset([item for item in batch_filter(supp, num_atoms, out_dim, data_type='mgf')], gzsupp, 
                                num_points=num_atoms, num_ms=out_dim, resolution=resolution, data_augmentation=data_augmentation)

    elif dataset == 'merge_infer': 
        supp = pd.read_csv(data_path)
        gzsupp = Chem.ForwardSDMolSupplier(gzip.open(mol_path)) # mol data 
        dataset = CSVDataset([item for item in batch_filter(supp, num_atoms, out_dim, data_type='csv')], gzsupp, 
                                num_points=num_atoms, num_ms=out_dim, resolution=resolution, data_augmentation=data_augmentation)
    else:
...
def batch_filter(supp, num_atoms=200, out_dim=2000, data_type='mgf'): 
